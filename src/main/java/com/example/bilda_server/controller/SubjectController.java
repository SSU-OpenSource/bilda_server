...
public class SubjectController {
    ...
    @Operation(summary = "유저가 속해있는 학과에 개설된 과목 정보 가져오기", description = "userId를 pathVariable로 넘기면 유저가 속해있는 과에 개설된 과목정보를 가져올 수 있습니다.  ", tags = {
        "SubjectController"})
    ...
    @Operation(summary = "유저가 속해 있는 과목 정보 가져오기", description = "userId를 pathVariable로 넘기면 유저가 듣고있는 과목정보를 가져올 수 있습니다.  ", tags = {
        "SubjectController"})
    @GetMapping("/{userId}")
    @ResponseBody
    ...
    @Operation(summary = "유저가 듣고 있는 과목 추가하기", description = "userId와 SubjectId를 pathVariable로 넘기면 유저가 듣고 있는 과목을 추가할 수 있습니다.  ", tags = {
        "SubjectController"})
    @PostMapping("/{userId}/add/{subjectCode}")
    public ResponseDto<User> addSubjectToUser(@PathVariable Long userId,
